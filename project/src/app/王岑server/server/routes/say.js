...
router.get('/', function (req, res, next) {
  ...
  if(JSON.stringify(req.query) !== "{}"){
    if(req.query.status && req.query.id){
      say.updateItem(req.query.status,req.query.id,(err,result)=>{
      ...
      */
      say.deleteSay(req.query.id,(err,result)=>{
        if(err){
          res.statusCode = 500;
        }else{
          res.send('已删除');
    ...
    say.getAll((err,result)=>{
      if(err){
        ...
        //console.log(obj);
        if(obj.length !==0 && obj[0].sid == null) obj = [];
        //console.log(obj);
      }
      res.render('say-list',{obj:obj});
    });
  } 
});

router.post('/',(req,res,next)=>{
  say.selectSay(req.body.scontent,(err,result)=>{
    if(err){
      res.statusCode = 500;
      return;
    }else{
