...
Photo.prototype.getAll = function(cb){
  const sql = 'select pid,pname,ptype,ptime,xname from Photo,Photos where Photo.xid = Photos.xid';
  db.query(sql,(err,result)=>{
    if(err){
      cb(true);
      return;
...
Photo.prototype.getPhoto = function(obj,cb){
  const sql = 'select pid,pname,ptype,ptime,xname from Photo,Photos where Photo.xid = Photos.xid and pid = ?';
